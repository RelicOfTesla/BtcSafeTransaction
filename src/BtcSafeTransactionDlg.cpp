...
double vol_to_amount( UINT64 v )
{
    return double( v ) / 100000000;
}
...
double WebQuery_GetBtcBalance_blockchain_info( const std::string& addr )
{
    ...
    return vol_to_amount( _strtoui64( str.c_str(), nullptr, 10 ) );
}
...
CRpcHelper::unspent_info WebQuery_GetUnspent_blockchain_info( const std::string& addr )
{
    CRpcHelper::unspent_info result;
    std::string explorer = "http://blockchain.info/unspent?active=";
    std::string str = HttpGet( explorer + addr );
    Json::Value jsv = json_from_string( str );
    if( jsv.size() )
    {
        jsv = jsv["unspent_outputs"];
		if (jsv.size())
		{
			for( Json::Value::iterator it = jsv.begin(); it != jsv.end(); ++it )
			{
				const Json::Value& v = *it;
				result.total += vol_to_amount( v["value"].asUInt() );
				CRpcHelper::unspent_info::unspent_txlist::value_type tx;
				tx.txid = CRpcHelper::txid_str( v["tx_hash"].asString() );
				tx.vout = v["tx_output_n"].asUInt();
				result.txlist.push_back( tx );
			}
		}
    }
    return result;
...
CRpcHelper::unspent_info WebQuery_GetUnspent( const std::string& addr )
{
    if( g_CoinAddr_FirstChar == '1' )
    {
        return WebQuery_GetUnspent_blockchain_info( addr );
    }
    // not support
